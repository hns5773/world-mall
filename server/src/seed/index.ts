...
const sampleProducts = [
  ...
  // Extra products for VIP4 (100 tasks) and VIP5 (120 tasks)
...
async function seed() {
  ...
  console.log('✅ Created admin user (admin / worldmall@2024)');

  // Create a sample sub-admin
  const subAdminPassword = await hashPassword('subadmin123');
  const subAdminInviteCode = generateInviteCode();
  const [subAdmin] = await db.insert(users).values({
    username: 'subadmin1',
    password: subAdminPassword,
    role: 'subadmin',
    inviteCode: subAdminInviteCode,
  }).returning();
  console.log(`✅ Created sub-admin (subadmin1 / subadmin123) with invite code: ${subAdminInviteCode}`);
  ...
  // Create orders per VIP level with proper task counts
  let totalOrders = 0;
  for (let level = 1; level <= 5; level++) {
  ...
  // Set global deposit addresses (USDT only, no BTC)
  ...
  console.log('✅ Set default deposit addresses (USDT TRC20 + ERC20)');
  ...
  console.log(`\n📋 Summary:`);
  console.log(`   Admin: admin / worldmall@2024`);
  console.log(`   Sub-Admin: subadmin1 / subadmin123 (Invite: ${subAdminInviteCode})`);
  console.log(`   VIP Levels: 1-5`);
  console.log(`   VIP1: 40 tasks, VIP2: 60, VIP3: 80, VIP4: 100, VIP5: 120`);
